...
int main(int argc, char **argv)
{
  ...
  MX_TIM3_Init();
  MX_TIM4_Init();
  rtmSetErrorStatus(Progetto_Gruppo21_IZ_M, 0);
  Progetto_Gruppo21_IZ_initialize();
  __disable_irq();
  ARMCM_SysTick_Config(modelBaseRate);
  runModel =
    rtmGetErrorStatus(Progetto_Gruppo21_IZ_M) == (NULL);
  __enable_irq();
  __enable_irq();
  while (runModel) {
    stopRequested = !(
  ...
  /* Terminate model */
  Progetto_Gruppo21_IZ_terminate();
